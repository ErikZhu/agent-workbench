  ...
  return (
    <div className="flex flex-col h-full" style={{ minHeight: 'calc(100vh - 180px)' }}>
      ...
      <div className="flex-shrink-0 px-6 pt-5 pb-3">
        ...
        <hr className="cyber-hr mt-3 max-w-3xl mx-auto" />
      </div>
...
function CouncilRoundView({ round, lang }: { round: CouncilRound; lang: Lang }) {
  const orderedResponses = [...round.responses].sort((a, b) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex justify-end">
        <div
          className="max-w-[65%] text-sm leading-relaxed whitespace-pre-wrap px-4 py-3"
          style={{
      ...
      {orderedResponses.map(resp => (
        <AgentMessage key={resp.agentId} response={resp} lang={lang} />
      ))}
...
// ── Agent message (WeChat-style left-aligned) ─────────────────────────────────
